...
void deleteRecord(fstream &data){
	...
	cout << "Delete number: ";
	cin >> number;
	// 2. in this position insert with empty data
	writeData(data,number,blankMovies);
	// 3. cek data in file data.bin, if data found
	// 	  move data to temporary file.
	dataTemp.open("temp.dat", ios::trunc | ios::out | ios::in | ios::binary);

	if(int i = 1; i <= size; i++){
		tempMovies = readData(data,i);

		if (tempMovies.title.empty()){
			writeData(dataTemp,i - offset,tempMovies);
		} else {
			offset++;
	...
	// 4. move data from temporary file to data.bin
	...
	for(int i = 1; i <= size; i++){
		writeData(data,i,readData(dataTemp,i));
	}
}
